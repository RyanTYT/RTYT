...
const stopWords = new Set(stopWord.split(" "));

function summaryBot() {
    this.verticies = [];
    this.dampeningFactor = 0.85;
    this.numVerticies = 0;
...
summaryBot.prototype.summary = function() {
    ...
    % workds kept: ${Math.round(((100 * this.infoOnLastRun.numWords) / origNumWords) * 100) /
            100
            }%
...
summaryBot.prototype.run = function(
    text,
    numReturnSentences,
    ...
    if (typeof text !== "string" || typeof numReturnSentences !== "number") {
        throw new TypeError(
            "ensure that you pass valild values into summaryBot.prototype.run"
        );
    }
...
summaryBot.prototype.getTopSentences = function(numSentences) {
    ...
    if (
        topSentencesArr.some(
            (vert) => vert.name === this.originalSentences.length - 1
        )
    ) {
        return out.join(". ");
    }
...
summaryBot.prototype._proccessString = function(text) {
    ...
    for (
        let sentence = 0;
        sentence < this.originalSentences.length;
        sentence++
    ) {
        this.filteredSentences[sentence] = this.originalSentences[
            sentence
        ].replace(/[^a-zA-Z0-9 \']/g, " ");
    }
    ...
    for (
        let sentence = 0;
        sentence < this.filteredSentences.length;
        sentence++
    ) {
        this._addVertex(
            this.filteredSentences[sentence]
                .toLowerCase()
                .split(" ")
                .filter(
                    (word) => word && word.length !== 1 && !stopWords.has(word)
                )
        );
    }
};
...
summaryBot.prototype._initialize = function() {
    ...
    for (
        let vertexNum = 0;
        vertexNum < this.verticies.length - 1;
        vertexNum++
    ) {
        for (
            let innerVertex = vertexNum + 1;
            innerVertex < this.verticies.length;
            innerVertex++
        ) {
            similarity = this._findSimilarity(
                this.verticies[vertexNum],
                this.verticies[innerVertex]
            );
...
summaryBot.prototype._updateAllVertexWeights = function() {
    ...
    for (let vertexNum = 0; vertexNum < this.numVerticies; vertexNum++) {
        for (
            let otherVertex = 0;
            otherVertex < this.numVerticies;
            otherVertex++
        ) {
            if (
                otherVertex !== vertexNum &&
                !this.unconnectedVerticies.has(otherVertex)
            ) {
                tempSum +=
                    (this.verticies[vertexNum].edge[otherVertex] /
                        totalEdgeWeights[otherVertex]) *
                    this.verticies[otherVertex].weight;
            }
        }
        this.verticies[vertexNum].weight =
            1 - this.dampeningFactor + this.dampeningFactor * tempSum;
        tempSum = 0;
    }
};
